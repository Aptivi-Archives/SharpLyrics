...
# Pack binary
echo Packing binary...
cd "../SharpLyrics/bin/$releaseconf/netstandard2.0/" && "$zippath" -r /tmp/$version-bin.zip . && cd -
cd "../SharpLyrics.Demo/bin/$releaseconf/net6.0/" && "$zippath" -r /tmp/$version-bin.zip . && cd -
if [ ! $? == 0 ]; then
